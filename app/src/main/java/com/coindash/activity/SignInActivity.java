...
public class SignInActivity extends BaseActivity {
    ...
    public void doSignIn(String UserName, String Password) {
        if (AppGlobal.isNetwork(SignInActivity.this)) {
            ...
            optioMap.put("EmailID", UserName);
            optioMap.put("Password", Password);
            optioMap.put("TokenData", AppGlobal.getId(SignInActivity.this));
                    ...
                    if (response.body() != null) {
                        if (response.body().getSuccess() == 1) {
                            ...
                            if (response.body().getInfo().getEmailVerificationStatus().equalsIgnoreCase("1")) {
                                ...
                                AppGlobal.setStringPreference(getApplicationContext(), response.body().getInfo().getFullname(), WsConstant.SP_LOGIN_NAME);
                                ...
                                AppGlobal.setStringPreference(getApplicationContext(), response.body().getInfo().getPhone(), WsConstant.SP_LOGIN_MOBILE);
                                ...
                                AppGlobal.setStringPreference(getApplicationContext(), response.body().getInfo().getCountry(), WsConstant.SP_COUNTRY);
...
//                                openDialogForResendLink(response.body().getInfo().getRegisterId());
    ...
    public void doForgotPassword(String Email) {
        if (AppGlobal.isNetwork(SignInActivity.this)) {
            ...
            new RestClient(SignInActivity.this).getInstance().get().ResetPassword(optioMap).enqueue(new Callback<CommonStatusResponse>() {
                @Override
